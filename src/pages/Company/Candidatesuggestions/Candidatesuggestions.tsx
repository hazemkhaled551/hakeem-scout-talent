  ...
  useEffect(() => {
    ...
    async function fetchCandidates() {
      try {
        ...
        const res = await recommendCandidates(jobId!);

        const recs = res.data.data.data.recommendation;

        // 🔥 نعمل mapping للداتا بتاعتك
        const formatted = recs.map((r: any) => ({
          id: r.id,
          name: r.applicant.user.name,
          role: `${r.applicant.job_title} · ${r.years_experience}yr`,
