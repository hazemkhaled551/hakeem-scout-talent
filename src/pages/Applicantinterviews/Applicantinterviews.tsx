...
type InterviewStatus =
  ...
  | "Cancelled"
  | "Rescheduled";
type InterviewType = "Technical" | "HR" | "Final" | "Culture Fit";
type ModalType = "view" | "cancel" | "feedback" | "offer" | null;
type OfferResponse = "accept" | "decline" | null;
...
type ExtendedInterviewDTO = Omit<InterviewDTO, "feedback"> & {
            ...
            {(activeTab === "upcoming" ? upcoming : past).map((iv, idx) => (
              ...
              >
                <div className="d-flex align-items-start gap-3 mb-3">
                  <div className="ai-company-logo">{companyIntiatal(iv?.application?.job?.company?.name)}</div>
                  <div className="flex-1 min-width-0">
                    <div className="d-flex align-items-start justify-content-between gap-2 flex-wrap mb-1">
                      <div>
                        <div className="ai-card-job">{iv.application.job.title}</div>
                        <div className="ai-card-company d-flex align-items-center gap-1">
                          <Building2 size={12} /> {iv?.application?.job?.company?.name}
                        </div>
                      </div>
